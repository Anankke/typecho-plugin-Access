        ...
        <div class="row typecho-page-main" role="main">
            <div class="col-mb-12">
                <ul class="typecho-option-tabs fix-tabs clearfix">
                    <li<?= ($access->action === 'overview' ? ' class="current"' : '') ?>><a
                                href="<?php $options->adminUrl('extending.php?panel=' . Access_Plugin::$panel . '&action=overview'); ?>"><?php _e('访问概览'); ?></a>
                    </li>
                    <li<?= ($access->action === 'logs' ? ' class="current"' : '') ?>><a
                                href="<?php $options->adminUrl('extending.php?panel=' . Access_Plugin::$panel . '&action=logs'); ?>"><?php _e('访问日志'); ?></a>
                    </li>
                    <li>
                        <a href="<?php $options->adminUrl('options-plugin.php?config=Access') ?>"><?php _e('插件设置'); ?></a>
                    </li>
                </ul>
            </div>
                            ...
                            <div class="search" role="search">
                                ...
                                <select name="filter">
                                    <option <?php if ($request->filter === 'all'): ?> selected="true"
                                                                                      <?php endif; ?>value="all"><?php _e('所有'); ?></option>
                                    <option <?php if ($request->filter === 'ip'): ?> selected="true"
                                                                                     <?php endif; ?>value="ip"><?php _e('按IP'); ?></option>
                                    <option <?php if ($request->filter === 'post'): ?> selected="true"
                                                                                       <?php endif; ?>value="post"><?php _e('按文章'); ?></option>
                                    <option <?php if ($request->filter === 'path'): ?> selected="true"
                                                                                       <?php endif; ?>value="path"><?php _e('按路由'); ?></option>
                                </select>
                                ...
                                <select style="<?php if ($request->get('filter', 'all') !== 'post'): ?>display: none<?php endif; ?>"
                                        name="cid">
                                    <?php foreach ($access->logs['cidList'] as $content): ?>
                                        <option <?php if ($request->cid == $content['cid']): ?> selected="true"
                                                                                                <?php endif; ?>value="<?php echo $content['cid']; ?>"><?php echo $content['title']; ?>
                                ...
                                <select name="type">
                                    <option <?php if ($request->type == 1): ?> selected="true"
                                                                               <?php endif; ?>value="1"><?php _e('默认(仅人类)'); ?></option>
                                    <option <?php if ($request->type == 2): ?> selected="true"
                                                                               <?php endif; ?>value="2"><?php _e('仅爬虫'); ?></option>
                                    <option <?php if ($request->type == 3): ?> selected="true"
                                                                               <?php endif; ?>value="3"><?php _e('所有'); ?></option>
                                </select>
                    ...
                    <form method="post" class="operate-form">
                        <div class="typecho-table-wrap">
                            <table class="typecho-list-table">
                                ...
                                <?php if (!empty($access->logs['list'])): ?>
                                    <?php foreach ($access->logs['list'] as $log): ?>
                                        <tr id="<?php echo $log['id']; ?>" data-id="<?php echo $log['id']; ?>">
                                            ...
                                            <td><a target="_self"
                                                   href="<?php $options->adminUrl('extending.php?panel=' . Access_Plugin::$panel . '&filter=path&path=' . $log['path'] . '&type=' . $request->type); ?>"><?php echo urldecode(str_replace('%23', '#', $log['url'])); ?></a>
                                            </td>
                                            <td><a data-action="ua" href="#"
                                                   title="<?php echo $log['ua']; ?>"><?php echo $log['display_name']; ?></a>
                                            </td>
                                            <td><a data-action="ip"
                                                   data-ip="<?php echo $log['ip']; ?>"
                                                   href="#"><?php echo $log['ip']; ?></a><?php if ($request->filter !== 'ip'): ?>
                                                <a target="_self"
                                                   href="<?php $options->adminUrl('extending.php?panel=' . Access_Plugin::$panel . '&filter=ip&ip=' . $log['ip'] . '&type=' . $request->type); ?>">
                                                        [ ? ]</a><?php endif; ?></td>
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="6"><h6 class="typecho-list-table-title"><?php _e('当前无日志'); ?></h6>
                                        </td>
                                    </tr>
                                <?php endif; ?>
                                </tbody>
